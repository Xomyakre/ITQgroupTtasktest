...
public class SubmitWorker {
    ...
    @Scheduled(fixedDelayString = "${app.workers.submit-fixed-delay-ms:5000}")
    public void run() {
        ...
        var docs = documentRepository.findNextBatchByStatus(DocumentStatus.DRAFT, PageRequest.of(0, batchSize));
        if (docs.isEmpty()) {
            return;
        }
        ...
        var results = documentService.submitBatch("SUBMIT-worker", ids, null);
        long ok = results.stream().filter(r -> "SUCCESS".equals(r.getResult())).count();
        log.info("SUBMIT-worker processed {} docs (success={}) in {} ms", ids.size(), ok, System.currentTimeMillis() - start);
    }
}
